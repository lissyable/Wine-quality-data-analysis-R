...
winedata_train_labels <- winequality[split_data, 9]
winedata_test_labels <- winequality[-split_data, 9]
```
...
```
As we can see, the model has the highest accuracy of 66% when k=1.Then the next highest accuracy is 60% when k=3 and k=5. When we repeat the process for k from 1 to 50 we see some decrease and then increase.
```{r}
accuracy <- rep(0, 50)
k <- 1:50
for(x in k){
